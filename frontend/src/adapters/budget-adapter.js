...
export const getBudgetsBySimulationId = async (simulationId) => {
  const [budgets, error] = await fetchHandler(`${baseUrl}/simulation/${simulationId}`);
  if (error) {
    console.log(error);
...
export const getBudgetBySimulationAndMonth = async (simulationId, currentMonth) => {
  const [budget, error] = await fetchHandler(`${baseUrl}/simulation/${simulationId}/month/${currentMonth}`);
  if (error) {
    console.log(error);
...
// Create a new budget
export const createBudget = async ({ simulationId, needs, wants, savings, lifeStyle }) => {
  return fetchHandler(baseUrl, getPostOptions({ simulationId, needs, wants, savings, lifeStyle }));
};
